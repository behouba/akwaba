...
type Storage struct {
	stmts          ordersStatments
	pricingService *pricing.Service
}

// New create new order storage, return error if it fail
func New(db *sqlx.DB, pricingService *pricing.Service) (*Storage, error) {
	stmts := ordersStatments{}
	if err := stmts.prepare(db); err != nil {
		return nil, err
	}

	return &Storage{stmts, pricingService}, nil
}
...
func (s *Storage) SaveOrder(ctx context.Context, o *akwaba.Order) (err error) {
	return s.stmts.saveOrder(ctx, o, s.pricingService)
}
