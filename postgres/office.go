...
func NewOfficeEmployeeStorage(db *sqlx.DB) *OfficeEmployeeStorage {
	return &OfficeEmployeeStorage{db: db}
}
...
func (es *OfficeEmployeeStorage) Authenticate(emp *akwaba.Employee) (employee akwaba.Employee, err error) {
	...
	err = es.db.QueryRow(
		`SELECT 
		e.first_name, e.last_name, e.password, e.office_id, o.name, o.area_id, a.name
		FROM employees AS e
		...
		&employee.Office.Name, &employee.Office.Area.ID,
		&employee.Office.Area.Name,
	)
	if err != nil {
		return
	}
