...
func (a *AdminDB) Pending(cityID int) (orders []akwaba.Order, err error) {
	rows, err := a.db.Query(
		`SELECT
		...
		WHERE state_id=$1 OR state_id=$2
		ORDER BY created_at DESC;`,
		akwaba.OrderStateWaitingConfirmation, akwaba.OrderStateInProcessing,
	)
	if err != nil {
		return
	}
...
func (a *AdminDB) ConfirmOrder(id int) (err error) {
	_, err = a.db.Exec(
		`UPDATE delivery_order SET state_id=$1 WHERE id=$2;`,
		akwaba.OrderStateInProcessing, id,
	)
	if err != nil {
		return
	}
