...
func EncodeParcelID(id int64) (trackID string, err error) {
	h, err := hashIDs()
	if err != nil {
		return
	}
	trackID, err = h.EncodeInt64([]int64{id})
	if err != nil {
		return
	}
	return
...
func DecodeTrackID(trackID string) (parcelID uint64, err error) {
	h, err := hashIDs()
	if err != nil {
		return
	}
	v, err := h.DecodeInt64WithError(trackID)
	if err != nil {
		return
	}
	parcelID = uint64(v[0])
